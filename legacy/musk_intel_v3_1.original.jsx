...
const C = {
  bg:"#050c1a", card:"#0d1629", nav:"#020810", border:"#1a2840",
  blue:"#1d6ff4", cyan:"#00c4e8", teal:"#00a878", green:"#12b76a",
  red:"#f04438", purple:"#7c5af0", amber:"#dc8f0a", coral:"#e84a2f",
  gray:"#4a5568", tx:"#e2e8f0", t2:"#94a3b8", t3:"#475569", bd2:"#1e2d45",
};
const SIG={极高:C.red,高:C.amber,中:C.blue,低:C.gray};
const SBG={极高:"#2d1010",高:"#2d1f08",中:"#0d1f3a",低:"#12161e"};
...
function Badge({text,color,bg}){
  return <span style={{display:"inline-block",padding:"2px 8px",borderRadius:12,fontSize:10,fontWeight:700,color,background:bg||color+"25",border:`1px solid ${color}40`,letterSpacing:"0.04em"}}>{text}</span>;
}
function Card({children,style}){
  return <div style={{background:C.card,borderRadius:12,padding:16,border:`1px solid ${C.border}`,marginBottom:12,boxShadow:"0 2px 16px rgba(0,0,0,0.4)",...style}}>{children}</div>;
}
function SL({children}){
  return <div style={{fontSize:9,fontWeight:800,color:C.t3,textTransform:"uppercase",letterSpacing:"0.18em",marginBottom:10,display:"flex",alignItems:"center",gap:6}}>{children}</div>;
}
function Stat({l,v,s,color,up}){
  return <div style={{background:"#0a1422",borderRadius:10,padding:"12px 14px",flex:1,minWidth:80,border:`1px solid ${C.border}`}}>
    <div style={{fontSize:9,color:C.t3,marginBottom:4,letterSpacing:"0.06em",textTransform:"uppercase"}}>{l}</div>
    <div style={{fontSize:20,fontWeight:800,color:color||C.tx,letterSpacing:"-0.5px",fontFamily:"'Courier New',monospace"}}>{v}</div>
    {s&&<div style={{fontSize:9,color:up===true?C.teal:up===false?C.red:C.t3,marginTop:3}}>{up===true?"▲ ":up===false?"▼ ":""}{s}</div>}
  </div>;
}
function Row({children}){return <div style={{display:"flex",gap:8,marginBottom:12,flexWrap:"wrap"}}>{children}</div>;}
function Bar2({val,max,color,h}){
  h=h||5;
  return <div style={{height:h,borderRadius:3,background:C.bd2}}>
    <div style={{height:"100%",borderRadius:3,background:color,width:`${Math.min(100,val/max*100)}%`,transition:"width 0.4s ease"}}/>
  </div>;
}
function Tabs({opts,val,set}){
  return <div style={{display:"flex",gap:3,marginBottom:14,flexWrap:"wrap",background:"#030a14",padding:4,borderRadius:10,border:`1px solid ${C.border}`}}>
    {opts.map(p=><button key={p} onClick={()=>set(p)} style={{padding:"5px 12px",borderRadius:8,fontSize:10,border:"none",background:val===p?"#1d6ff4":"transparent",color:val===p?"#fff":C.t2,cursor:"pointer",fontWeight:val===p?800:400,letterSpacing:"0.03em",transition:"all 0.15s"}}>{p}</button>)}
  </div>;
}
function Legal({isZH}){
  return <div style={{background:"#051a0e",border:"1px solid #00a87840",borderRadius:8,padding:"6px 14px",marginBottom:12,fontSize:9,color:C.teal,fontWeight:600,letterSpacing:"0.04em"}}>
...
function Overview({isZH,t}){
  ...
  return <div>
    <Legal isZH={isZH}/>
    <div style={{background:"linear-gradient(135deg,#0d1a38,#1a0d38)",borderRadius:14,padding:"20px 22px",marginBottom:14,border:`1px solid #2a3070`}}>
      <div style={{fontSize:9,fontWeight:800,color:"#60a5fa",letterSpacing:"0.22em",marginBottom:8}}>MUSK INTEL · {L("情报摘要","INTEL SUMMARY",isZH)} · 04/26/2026 ◉ LIVE</div>
      <div style={{fontSize:15,fontWeight:800,color:C.tx,lineHeight:1.5,letterSpacing:"-0.2px"}}>
    ...
    <Card>
      ...
      {evs.map(e=>(
        <div key={e.id} style={{display:"flex",gap:10,marginBottom:12,paddingBottom:12,borderBottom:`1px solid ${C.bd2}`}}>
          <div style={{width:3,background:e.col,borderRadius:2,flexShrink:0}}/>
          <div style={{flex:1,minWidth:0}}>
            <div style={{display:"flex",gap:6,flexWrap:"wrap",marginBottom:4,alignItems:"center"}}>
              <Badge text={isZH?e.tyZH:e.tyEN} color={e.col}/>
              <Badge text={e.tag} color={C.gray} bg="#0a1422"/>
              <span style={{fontSize:9,color:C.t3,marginLeft:"auto"}}>{e.date}</span>
            </div>
            <div style={{fontSize:11,color:C.tx,lineHeight:1.6}}>{isZH?e.tiZH:e.tiEN}</div>
          </div>
          <Badge text={t.sig[e.sig]} color={SIG[e.sig]} bg={SBG[e.sig]}/>
        </div>
      ))}
    </Card>
    <Card style={{background:"#051a0e",border:`1px solid ${C.teal}30`}}>
      <SL>{L("⚠ 免责声明","⚠ Disclaimer",isZH)}</SL>
      <div style={{fontSize:10,color:C.teal,lineHeight:1.7}}>{L("以上内容仅为基于公开信息（SEC EDGAR·ADS-B·X公开账号·公开新闻）的信号推演，不构成专业投资建议。投资有风险，决策需谨慎。","Above content is public data signal inference only (SEC EDGAR·ADS-B·X public·News). Not professional investment advice. Invest at your own risk.",isZH)}</div>
    </Card>
  </div>;
}
...
function Market({isZH,t,cr}){
  ...
  return <div>
    ...
    <Card>
      ...
      <Candle data={KD[s.k]}/>
      <div style={{marginTop:8,padding:"8px 12px",background:"#0a1422",borderRadius:8,fontSize:11,color:C.t2}}>{s.note}</div>
    </Card>
    <Card>
...
function XPlatform({isZH}){
  ...
  return <div>
    ...
    {sub===1&&<Card>
      ...
      <div style={{background:"#0a1422",borderRadius:8,padding:"8px 12px",marginTop:8,fontSize:10,color:C.t2,lineHeight:1.7}}>
    ...
    {sub===2&&<Card>
      ...
      <div style={{background:"#051a0e",borderRadius:8,padding:"8px 12px",marginTop:8,fontSize:10,color:C.teal,lineHeight:1.7}}>
...
function Travel({isZH}){
  ...
  return <div>
    ...
    <Card>
      ...
      <div style={{background:SBG["极高"],borderRadius:8,padding:"8px 12px",marginTop:8,fontSize:10,color:"#fca5a5",lineHeight:1.7,border:`1px solid ${C.red}30`}}>
...
function Space({isZH}){
  return <div>
    <Legal isZH={isZH}/>
    <div style={{background:"linear-gradient(135deg,#030e1e,#042C53)",borderRadius:12,padding:"16px 20px",marginBottom:12,border:`1px solid #0c447c`}}>
      <div style={{fontSize:14,fontWeight:800,color:"#e2e8f0",marginBottom:4}}>{L("太空探索专项追踪","Space Exploration Intel",isZH)}</div>
      <div style={{fontSize:10,color:"rgba(255,255,255,0.5)"}}>SpaceX · Starship · Starlink · NASA合同 · 火星计划</div>
    </div>
    <Row>
    ...
    <Card style={{background:"#030e1e",border:`1px solid #0c447c`}}>
      <SL><span style={{color:"#7dd3fc"}}>{L("火星计划公开时间表（Musk公开言论+FAA申报）","Mars Timeline (Public Statements + FAA Filings)",isZH)}</span></SL>
      {[{y:"2026",e:L("Starship无人火星任务（FAA许可申请中）","Starship uncrewed Mars mission (FAA permit pending)",isZH),conf:82},{y:"2027",e:L("首次载人火星任务（初步设计完成）","First crewed Mars mission (prelim design complete)",isZH),conf:64},{y:"2029",e:L("火星基地建设·Optimus机器人部署","Mars base construction with Optimus robots",isZH),conf:46},{y:"2040",e:L("火星永久居住·目标1万人","Mars permanent settlement, 10K people target",isZH),conf:28}].map((p,i)=><div key={i} style={{marginBottom:12}}>
        <div style={{display:"flex",justifyContent:"space-between",marginBottom:4}}>
          <span style={{fontSize:13,fontWeight:800,color:"#7dd3fc",fontFamily:"'Courier New',monospace"}}>{p.y}</span>
          <span style={{fontSize:12,fontWeight:700,color:p.conf>=70?C.teal:p.conf>=50?C.amber:C.gray,fontFamily:"'Courier New',monospace"}}>{p.conf}%</span>
        </div>
        <div style={{fontSize:10,color:"rgba(255,255,255,0.65)",lineHeight:1.5,marginBottom:4}}>{p.e}</div>
...
function Anomaly({isZH,t}){
  return <div>
    <Legal isZH={isZH}/>
    <div style={{background:SBG["极高"],borderRadius:10,padding:"12px 16px",marginBottom:12,border:`1px solid ${C.red}35`}}>
      <div style={{fontSize:10,fontWeight:800,color:C.red,marginBottom:4,letterSpacing:"0.05em"}}>⚠ {L("信号收敛分析系统 · ACTIVE · 04/26 2026","Signal Convergence Analysis System · ACTIVE · 04/26 2026",isZH)}</div>
    ...
    {ANOMDATA.map((a,i)=><Card key={i} style={{border:`1.5px solid ${SIG[a.sev]}35`,background:SBG[a.sev]}}>
      <div style={{display:"flex",justifyContent:"space-between",marginBottom:8,alignItems:"flex-start"}}>
        <div>
          <div style={{fontSize:13,fontWeight:800,color:C.tx}}>{isZH?a.tyZH:a.tyEN}</div>
          <div style={{fontSize:9,color:C.t3,marginTop:2}}></div>
        </div>
        <div style={{textAlign:"right"}}>
          <Badge text={t.sig[a.sev]} color={SIG[a.sev]} bg={SBG[a.sev]}/>
          <div style={{fontSize:20,fontWeight:900,color:SIG[a.sev],fontFamily:"'Courier New',monospace",marginTop:3}}>{a.conv}%</div>
          <div style={{fontSize:8,color:C.t3}}>{L("汇聚置信度","Convergence",isZH)}</div>
        </div>
      </div>
      <Bar2 val={a.conv} max={100} color={SIG[a.sev]} h={5}/>
      <div style={{fontSize:11,color:C.t2,lineHeight:1.7,marginTop:10}}>{isZH?a.deZH:a.deEN}</div>
      <div style={{display:"flex",gap:5,flexWrap:"wrap",marginTop:8}}>
        {a.layers.map((l,j)=><Badge key={j} text={l} color={C.blue} bg="#0d1629"/>)}
      </div>
      <div style={{marginTop:8,fontSize:9,color:C.teal,background:"#051a0e",borderRadius:6,padding:"5px 10px",fontWeight:700}}>📊 {a.hist}</div>
    </Card>)}
  </div>;
}
...
function Predict({isZH}){
  return <div>
    <Legal isZH={isZH}/>
    <Card style={{border:`1px solid ${C.purple}35`}}>
      <SL>{L("综合预测框架","Prediction Framework",isZH)}</SL>
      <div style={{fontSize:10,color:C.t2,lineHeight:1.8}}>{L("基于：SEC申报时序 · 招聘领先指标 · 出行-决策相关性 · 社媒情绪极值 · 关系圈资本动向 · 专利→产品滞后分析。概率=历史类似模式贝叶斯后验估计。","Based on: SEC filing temporal analysis · Recruiting leading indicators · Travel-decision correlation · Social sentiment extremes · Inner circle capital moves · Patent-to-product lag analysis. Probability = Bayesian posterior from historical pattern hit rates.",isZH)}</div>
    </Card>
    <Card>
      <SL>{L("未来2-4周预测事件","Next 2-4 Week Predicted Events",isZH)}</SL>
      {PRED.map((item,i)=><div key={i} style={{marginBottom:12,padding:12,borderRadius:10,background:"#0a1422",border:`1px solid ${item.p>=80?C.coral:item.p>=65?C.amber:C.gray}25`}}>
        <div style={{display:"flex",justifyContent:"space-between",marginBottom:6}}>
          <span style={{fontSize:9,color:C.t3,fontWeight:700}}>#{i+1}</span>
          <span style={{fontSize:20,fontWeight:900,color:item.p>=80?C.coral:item.p>=65?C.amber:C.gray,fontFamily:"'Courier New',monospace"}}>{item.p}%</span>
        </div>
        <div style={{fontSize:11,color:C.tx,lineHeight:1.6,marginBottom:6}}>{item.eZH}</div>
        <Bar2 val={item.p} max={100} color={item.p>=80?C.coral:item.p>=65?C.amber:C.gray} h={4}/>
      </div>)}
    </Card>
    <Card>
...
function Weekly({isZH,t}){
  ...
  return <div>
    <div style={{background:"linear-gradient(135deg,#070d1a,#0d1a38,#1a0d38)",borderRadius:14,padding:"22px",marginBottom:14,border:`1px solid #2a2a70`}}>
      <div style={{fontSize:9,fontWeight:800,color:"#60a5fa",letterSpacing:"0.22em",marginBottom:8}}>MUSK INTEL · {L("周度全局分析","WEEKLY GLOBAL REPORT",isZH)} · 04/20–04/26 2026</div>
      <div style={{fontSize:16,fontWeight:900,color:C.tx,lineHeight:1.45,letterSpacing:"-0.3px"}}>
    ...
    <Card>
      ...
      {recs.map((inv,i)=><div key={i} style={{marginBottom:10,padding:12,borderRadius:10,background:"#0a1422",border:`1px solid ${inv.c}25`}}>
        <div style={{display:"flex",justifyContent:"space-between",marginBottom:5}}>
          <div style={{display:"flex",gap:8,alignItems:"center"}}>
            <Badge text={inv.tZH} color={inv.c}/>
            <span style={{fontSize:12,fontWeight:700,color:C.tx}}>{inv.target}</span>
          </div>
          <span style={{fontSize:9,color:C.t3,fontFamily:"'Courier New',monospace"}}>置信度 {inv.conf}%</span>
        </div>
        <div style={{fontSize:11,color:C.t2,marginBottom:5}}>{inv.rZH}</div>
        <Bar2 val={inv.conf} max={100} color={inv.c} h={3}/>
      </div>)}
    </Card>
    <Card style={{background:"#150a0a",border:`1px solid ${C.red}30`}}>
      ...
      {[L("中美科技脱钩加速：北京出行若涉及敏感技术，监管介入将影响Tesla中国$8B年营收","US-China tech decoupling: sensitive tech talks risk regulatory intervention in Tesla China $8B revenue",isZH),L("AI监管矛盾：欧盟AI法案与美国AI加速法案可能冲突，影响xAI欧洲扩张","AI regulation contradiction: EU AI Act vs US AI Acceleration Act may conflict; affects xAI Europe expansion",isZH),L("Tesla能源分拆执行风险：税务+股东+监管三重阻力，可能错过最佳市场窗口","Tesla energy spinoff execution: tax+shareholder+regulatory triple friction; may miss optimal market window",isZH),L("SpaceX前流动性受限：$520B估值无公开变现，二级市场折价风险","SpaceX pre-IPO liquidity: $520B valuation with no public exit; secondary discount risk",isZH)].map((r,i)=><div key={i} style={{display:"flex",gap:8,marginBottom:8}}>
        ...
        <span style={{fontSize:11,color:"#fca5a5",lineHeight:1.6}}>{r}</span>
      </div>)}
    </Card>
  </div>;
}
...
export default function App(){
  ...
  return (
    <div style={{fontFamily:"'SF Pro Display','Segoe UI',system-ui,sans-serif",background:C.bg,minHeight:"100vh",color:C.tx}}>
      <style>{`@keyframes pulse{0%,100%{opacity:1}50%{opacity:0.5}} ::-webkit-scrollbar{width:4px;height:4px} ::-webkit-scrollbar-track{background:#030a14} ::-webkit-scrollbar-thumb{background:#1a2840;borderRadius:2px}`}</style>
      <div style={{background:C.nav,borderBottom:"1px solid #0d1629",position:"sticky",top:0,zIndex:100,boxShadow:"0 2px 20px rgba(0,0,0,0.5)"}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"0 16px",height:52}}>
          <button onClick={()=>setTab("overview")} style={{border:"none",background:"none",cursor:"pointer",padding:0,display:"flex",alignItems:"center",gap:10}}>
            <div style={{width:28,height:28,borderRadius:7,background:"linear-gradient(135deg,#1d6ff4,#7c5af0)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:10,fontWeight:900,color:"#fff"}}>MI</div>
            <span style={{fontSize:14,fontWeight:900,color:"#fff",letterSpacing:"0.1em",fontFamily:"'Courier New',monospace"}}>MUSK<span style={{color:C.cyan}}>INTEL</span></span>
            <span style={{fontSize:8,color:C.t3}}>v3.0 · 14L</span>
          </button>
          <div style={{display:"flex",alignItems:"center",gap:8}}>
            <button onClick={()=>setLang(isZH?"EN":"ZH")} style={{padding:"4px 10px",borderRadius:7,border:"1px solid #1a2840",background:"#0d1629",color:C.cyan,cursor:"pointer",fontSize:10,fontWeight:700}}>🌐 {t.langBtn}</button>
            <div style={{display:"flex",alignItems:"center",gap:5,fontSize:8,color:C.t3}}>
              <span style={{width:5,height:5,borderRadius:"50%",background:C.teal,display:"inline-block",boxShadow:`0 0 6px ${C.teal}`,animation:"pulse 2s infinite"}}/>
              {now}
            </div>
          </div>
        </div>
        <div style={{display:"flex",padding:"0 12px",borderTop:"1px solid #0d1629",overflowX:"auto"}}>
          {NAV_IDS.map(id=><button key={id} onClick={()=>setTab(id)} style={{padding:"9px 12px",border:"none",borderBottom:`2px solid ${tab===id?C.cyan:"transparent"}`,background:"transparent",color:tab===id?C.cyan:C.t3,cursor:"pointer",fontWeight:tab===id?700:400,fontSize:10,whiteSpace:"nowrap",display:"flex",alignItems:"center",gap:4,transition:"all 0.15s",letterSpacing:"0.03em"}}>
            {t.nav[id]}
            {id==="anomaly"&&<span style={{background:C.red,color:"#fff",borderRadius:8,fontSize:8,padding:"1px 5px",fontWeight:800}}>4</span>}
            {id==="weekly"&&<span style={{background:C.blue,color:"#fff",borderRadius:8,fontSize:8,padding:"1px 5px",fontWeight:800}}>★</span>}
            {id==="xp"&&<span style={{background:C.purple,color:"#fff",borderRadius:8,fontSize:8,padding:"1px 5px",fontWeight:800}}>AI</span>}
          </button>)}
        </div>
      </div>
      <div style={{maxWidth:780,margin:"0 auto",padding:"14px"}}>{render()}</div>
    </div>
  );
}
