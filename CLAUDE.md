...
## 协作纪律 — 极重要
- 面向非技术维护者：主动做专业性普及——每个改动涉及的行业术语与原理顺带讲清（如 Treemap、二元事件、饱和度对比）。
- 全程正式书面语，禁口语痞气；结论先行；不奉承不邀功。判断权在维护者，但分析结论该顶就顶（拒绝不合理需求并给出理由）。
- 视觉红线（违反即返工）：孤字、错位、数字序号编码、图大字小比例失衡、SVG 中文、高对比纯色、口语化文案、C 级出现在展示层、无出处的数字。
- 每轮先出方案对齐再动手；被打断时先复述确认理解再执行。
