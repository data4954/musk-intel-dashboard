...
window.MUSKINTEL_DATA = {
 ...
 "pol_lens": [
  {
   "lens": "宏观 · 财政与产业政策",
   "mech": "三条传导管道决定成本地板与需求基座：其一，IRA 补贴与关税结构直接进入储能与整车的单位经济——补贴变动等于成本曲线整体平移，这解释了为何国会预算案是 L4 级先行指标；其二，利率环境决定长久期叙事资产的贴现率敏感度——TSLA 溢价 93% 押在 2030 年后的现金流上，久期极长，对利率的 beta 远高于其车业务本身；其三，联邦采购周期（NASA/DoD）是 SPCX 收入的政府基座，预算年度节奏给了它准财政属性。",
   "read": "当前态：补贴中性、采购顺风；真正的宏观敞口是利率——叙事溢价是本质上的长久期零息债。",
   "watch": "国会预算案 · 采购授权法案 · 实际利率走势"
  },
  {
   "lens": "微观 · 监管个案经济学",
   "mech": "每个监管案子都是一条边际成本曲线的行政段：NHTSA 三项调查=Robotaxi 每辆车的隐性许可成本，裁决前无法摊薄；FAA 许可节拍=星舰试飞频率的行政上限，直接卡复用学习曲线的迭代速度；孟菲斯燃气轮机诉讼=Colossus 扩张的边际电力成本在法庭上定价。微观视角的要点：这些不是'风险'，是已经在发生的成本，只是记在监管账上而非损益表上。",
   "read": "当前态：三案全部悬而未决——悬置本身压制资本开支的回报确定性。",
   "watch": "逐案裁决日 · 州级备案（先行绕行路径）"
  },
  {
   "lens": "政治经济学 · 规制俘获与共生结构",
   "mech": "马斯克系是教科书之外的三位一体：政府承包商（收入依赖）+规制对象（调查在身）+舆论平台持有者（X）。承包商身份给它'大而不能倒'的运力人质（NASA 载人无替代），规制对象身份给政府惩罚抓手，舆论平台给它反向威慑工具——三重身份互为抵押品，构成双向规制俘获：政府难以彻底惩罚它，它也无法彻底摆脱政府。主权资本入股（HUMAIN/QIA/MGX）是第四层：地缘租金入股换取市场准入与政治缓冲，代价是 CFIUS 敞口。旋转门（Sacks 入白宫）把私人关系制度化为政策通道。",
   "read": "当前态：共生均衡运行中；这个结构的脆弱点不在任何单一案子，在委托人更替（选举）导致整个抵押品网络重新定价。",
   "watch": "人事任免 · 主权持股听证 · 合同集中度变化"
  },
  {
   "lens": "博弈论 · 重复博弈与相互威慑",
   "mech": "2025-05~07 的决裂-报复-和解是一轮完整的博弈展示：双方各持人质（政府合同↔载人运力；调查权+合同审查↔舆论动员+爆料能力），决裂期双方都动用了惩罚路径（NASA 提名撤回、空军叫停 vs 建党威胁、档案爆料），随后回到合作均衡——标准的以牙还牙后修复。当前的金援是可置信承诺信号（沉没成本型），'美国党'未注销则是保留在手的威慑期权。关键结构：这是有限期重复博弈——2028 年委托人任期结束是已知终点，博弈论预言临近终点时合作激励衰减，背叛概率上升。",
   "read": "当前态：合作均衡+双向威慑在架；11-03 是外生冲击点——委托人权力削弱会直接改变支付矩阵。",
   "watch": "'美国党'注销与否（威慑期权状态）· 金援投放节奏 · 终局临近期的合作衰减征兆"
  },
  {
   "lens": "投资学 · 政治风险定价",
   "mech": "政治敞口在两标的上是不对称的期权结构：TSLA 的政治 beta 集中在监管路径（FSD 联邦框架/NHTSA），表现为二元事件期权——赛前隐含波动率抬升、赛后单边释放；SPCX 的政治 beta 集中在合同与审查（采购+CFIUS），表现为缓变的信用利差型风险。定价纪律：二元事件不该用均值预期定价，该用情景加权——这正是情景树纪律的投资学依据。对冲上，两标的政治风险同源（同一委托人关系），分散无效，只能用时间敞口管理（事件前减、事件后按情景执行）。",
   "read": "当前态：距头号二元事件的定价窗口正在收窄；同源风险意味着 TSLA+SPCX 双持仓的政治敞口是叠加而非分散。",
   "watch": "事件前隐含波动率 · 情景树执行纪律 · 两标的相关性"
  }
 ],
 "travel": {
  "channel_note": "ADS-B 免费公开源已收费，自动通道暂停；当前通道=公开报道人工入库+人物新闻抓取。边界：仅本人公务机公开广播与公开报道行程，不涉私人生活。",
  "verified": [
   {
    "d": "2025-04",
    "dest": "北京",
